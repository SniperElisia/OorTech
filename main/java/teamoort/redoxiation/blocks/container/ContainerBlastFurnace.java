...
public class ContainerBlastFurnace extends Container {
    ...
    // slot index is the unique index for all slots in this container i.e. 0 - 35 for invPlayer then 36 - 49 for tileBlastFurnace
    ...
    // slot number is the slot number within each component; i.e. invPlayer slots 0 - 35, and tileBlastFurnace slots 0 - 14
    ...
    public ContainerBlastFurnace(InventoryPlayer invPlayer, TileBlastFurnaceBlock tileBlastFurnace) {
        this.tileBlastFurnace = tileBlastFurnace;

        final int SLOT_X_SPACING = 18;
        final int SLOT_Y_SPACING = 18;
        final int HOTBAR_XPOS = 8;
        final int HOTBAR_YPOS = 194;
        ...
        final int PLAYER_INVENTORY_YPOS = 136;
        ...
        final int FUEL_SLOTS_XPOS = 43;
        final int FUEL_SLOTS_YPOS = 82;
        // Add the tile fuel slots
        for (int x = 0; x < FUEL_SLOTS_COUNT; x++) {
            int slotNumber = x + FIRST_FUEL_SLOT_NUMBER;
            addSlotToContainer(new SlotFuel(tileBlastFurnace, slotNumber, FUEL_SLOTS_XPOS + SLOT_X_SPACING * x, FUEL_SLOTS_YPOS));
        }
        ...
        for (int x = 0; x < INPUT_SLOTS_COUNT; x++) {
            ...
            addSlotToContainer(new SlotSmeltableInput(tileBlastFurnace, slotNumber, INPUT_SLOTS_XPOS + SLOT_X_SPACING * x, INPUT_SLOTS_YPOS));
        }

        final int OUTPUT_SLOTS_XPOS = 90;
        final int OUTPUT_SLOTS_YPOS = 115;
        // Add the tile output slots
        for (int x = 0; x < OUTPUT_SLOTS_COUNT; x++) {
            int slotNumber = x + FIRST_OUTPUT_SLOT_NUMBER;
            addSlotToContainer(new SlotOutput(tileBlastFurnace, slotNumber, OUTPUT_SLOTS_XPOS + 26 * x, OUTPUT_SLOTS_YPOS));
        }
    }
    ...
    public void detectAndSendChanges() {
        ...
        boolean fieldHasChanged [] = new boolean[tileBlastFurnace.getFieldCount()];
        if (cachedFields == null) {
            cachedFields = new int[tileBlastFurnace.getFieldCount()];
            allFieldsHaveChanged = true;
        }
        for (int i = 0; i < cachedFields.length; ++i) {
            if (allFieldsHaveChanged || cachedFields[i] != tileBlastFurnace.getField(i)) {
                cachedFields[i] = tileBlastFurnace.getField(i);
        ...
        for (int i = 0; i < this.crafters.size(); ++i) {
            ...
            for (int fieldID = 0; fieldID < tileBlastFurnace.getFieldCount(); ++fieldID) {
    ...
    public void updateProgressBar(int id, int data) {
        tileBlastFurnace.setField(id, data);
    }
    ...
    public class SlotOutput extends Slot {
        ...
        @Override
        public boolean isItemValid(ItemStack stack) {
            return TileBlastFurnaceBlock.isItemValidForOutputSlot(stack);
        }
    }
}
