...
public class TileBlastFurnaceBlock extends TileEntity implements IInventory, IUpdatePlayerListBox{
    ...
    public boolean isUseableByPlayer(EntityPlayer player) {
        if (this.worldObj.getTileEntity(xCoord, yCoord, zCoord) != this) return false;
        final double X_CENTRE_OFFSET = 0.5;
        final double Y_CENTRE_OFFSET = 0.5;
        final double Z_CENTRE_OFFSET = 0.5;
        final double MAXIMUM_DISTANCE_SQ = 8.0 * 8.0;
        return player.getDistanceSq(xCoord + X_CENTRE_OFFSET, yCoord + Y_CENTRE_OFFSET, zCoord + Z_CENTRE_OFFSET) < MAXIMUM_DISTANCE_SQ;
    }
