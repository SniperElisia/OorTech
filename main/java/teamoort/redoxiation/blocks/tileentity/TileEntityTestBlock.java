...
public class TileEntityTestBlock extends TileEntity {
			...
			for (int y = yCoord; y < yCoord + 3; y++)
				for (int z = zCoord - 1; z < zCoord + 2; z++) {
					...
					if (tile != null && (tile instanceof TileEntityTestBlock)) {
						if (this.isMaster()) {
							if (((TileEntityTestBlock)tile).hasMaster())
								i++;
						} else if (!((TileEntityTestBlock)tile).hasMaster())
							i++;
					}
			}
        ...
        for (int x = xCoord - 1; x < xCoord + 2; x++)
            for (int y = yCoord; y < yCoord + 3; y++)
                for (int z = zCoord - 1; z < zCoord + 2; z++) {
                    ...
                    if (tile != null && (tile instanceof TileEntityTestBlock)) {
                        ((TileEntityTestBlock) tile).setMasterCoords(xCoord, yCoord, zCoord);
                        ((TileEntityTestBlock) tile).setHasMaster(true);
                        ((TileEntityTestBlock) tile).setIsMaster(master);
                    }
                }
    }
    ...
    public boolean checkForMaster() {
        ...
        return (tile != null && (tile instanceof TileEntityTestBlock));
    }
        ...
        for (int x = xCoord - 1; x < xCoord + 2; x++)
            for (int y = yCoord; y < yCoord + 3; y++)
                for (int z = zCoord - 1; z < zCoord + 2; z++) {
                    TileEntity tile = worldObj.getTileEntity(x, y, z);
                    if (tile != null && (tile instanceof TileEntityTestBlock))
                        ((TileEntityTestBlock) tile).reset();
                }
    }
